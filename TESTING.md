...
| --- | --- | --- | --- |
| ☐ **1. Does the app open at all** | Install this build and open it | It opens and stays open | **Do this first and tell me either way.** The app opening and closing instantly was my fault: a test that decides whether the N64 is possible ran automatically at startup, and that test writes instructions into memory and runs them, which iOS shuts an app down for unless the permission survived however you installed it. It no longer runs at startup. If this build STILL closes instantly, it is not that, and the next suspect is the signing certificate, so tell me which installer you use |
| ☐ **1b. The JIT test, on purpose** | Only once the app opens: Settings → BIOS section → **Test whether this build can run generated code** | Either a result appears on the JIT line, or the app closes | **The app closing here is a real answer, not a crash**, and reopening is completely safe. It means the permission did not survive your install method. If it stays open, read the JIT line above the button and send it to me. This decides whether the Nintendo 64 is possible at all |
